...
When /^I connect to that endpoint$/ do
  steps %Q{
    When I run `syslog-shipper -s #{@host_name}:#{@port} *` interactively
  }
end
...
When /^I securely connect to that endpoint$/ do
  ...
  steps %Q{
    When I run `syslog-shipper --ca-cert #{file_path} -s #{@host_name}:#{@port} *` interactively
  }
end

When /^I connect to that endpoint with and bypass peer checking$/ do
  steps %Q{
    When I run `syslog-shipper --skip-peer-check -s #{@host}:#{@port} *` interactively
  }
end
